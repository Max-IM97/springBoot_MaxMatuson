...
public class CatDAOImpl implements CatDAO {
	...
	public List<Cats> findAll() {
		...
		RowMapper<Cats> rowMapper = new BeanPropertyRowMapper<Cats>(Cats.class);	

		return jdbcTemplate.query(sql, rowMapper);
	}

	public Optional<Cats> findById(Long id) {
		Optional<Cats> oCat = Optional.ofNullable(null);
		...
		if (id == null) 
			...
			RowMapper<Cats> rowMapper = new BeanPropertyRowMapper<Cats>(Cats.class);	
			Cats cats = jdbcTemplate.queryForObject(sql, rowMapper, id);
	...
	public Optional<Cats> findCat(Cats cats) {
		Optional<Cats> oCat = Optional.ofNullable(null);
		...
		if (cats != null) {
			...
			if (id != null)
				...
				List<Cats> cats1 = findByNameAndType(name, type);
	...
	public Cats save(Cats cats) {
		...
		if (cats != null) {
			...
			logger.warn("Unexepected null input in save(). Returning null");
	...
	public void delete(Cats cats) {
		if (cats != null) {
			...
			if (id == null) {
				Optional<Cats> oCat = findCat(cats);
	...
	public boolean exists(Cats cats) {
	...
	public Cats add(Cats cats, boolean updateIfExists) {
		if (cats != null) {
			if (exists(cats)) {
				if (updateIfExists) {
					delete(cats);
	...
	public List<Cats> findByNameAndType(String name, String type) {
		String sql = "SELECT e.id, e.ame, e.type FROM cats e WHERE e.name = ? AND e.type = ?";
		RowMapper<Cats> rowMapper = new CatRowMapper();
		
		return jdbcTemplate.query(sql, rowMapper, name, type);
	}

	public List<Cats> findByNameOrType(String name, String type) {
